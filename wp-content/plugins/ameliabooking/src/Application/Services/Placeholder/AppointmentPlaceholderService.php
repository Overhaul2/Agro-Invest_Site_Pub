...
class AppointmentPlaceholderService extends PlaceholderService
{
    ...
    private function getServiceData($appointmentArray, $bookingKey = null)
    {
        ...
        $categoryName = $helperService->getBookingTranslation(
            $bookingKey !== null ? $helperService->getLocaleFromBooking($appointmentArray['bookings'][$bookingKey]['info']) : null,
        ...
        foreach ((array)$appointmentArray['bookings'] as $key => $booking) {
            if (($bookingKey === null &&
                    ($booking['isChangedStatus'] || $booking['status'] === BookingStatus::APPROVED || $booking['status'] === BookingStatus::PENDING)) || $bookingKey === $key) {
    ...
    public function getEmployeeData($appointment, $bookingKey = null)
    {
        ...
        if ($location) {
            $locationName = $helperService->getBookingTranslation(
                $bookingKey !== null ? $helperService->getLocaleFromBooking($appointment['bookings'][$bookingKey]['info']) : null,
    ...
    public function getGroupedAppointmentData($appointment, $bookingKey, $type)
    {
        ...
        foreach ($appointment['bookings'] as $bookingId => $booking) {
            ...
            $groupPlaceholders = array_merge(
                ...
                $this->getServiceData($appointment, $bookingId),
